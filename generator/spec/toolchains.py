...
cross-builds for, and which compiler/apt packages each one needs.

Used by build.py: ALL_ARCHES drives `--all` (build every architecture
whose compiler is found on PATH) and `--docker` (installs every listed
apt package in an ephemeral image, then runs `--all` inside it).
...
# android-arm64/android-armv7 targets. Bump this to move to a newer NDK;
# generator/ndk.py matches by major revision number (the leading digits),
# so a locally installed r27a/r27b/etc. still counts as "the same" NDK
# and won't trigger a redundant download.
NDK_VERSION = "r28b"

# dist-name -> (cc, ar, [apt packages needed to get that cc/ar + its libc])
ALL_ARCHES: dict[str, dict[str, object]] = {
